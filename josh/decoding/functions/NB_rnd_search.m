function [mi_cumulative,mae_cumulative,timecourse_results,cells_chosen] = NB_rnd_search(population,n_searches,search_depth,timecourse_save_steps,... 
                                                                   mIdx,i_session,iVar,iRewsize,iFeature,... 
                                                                   X_dataset,y_dataset,models,xval_table,dataset_opt)
% Randomly add cells to included population one-by-one
    ...
    cells_chosen = cell(n_searches,1); 

    h0 = waitbar(0,sprintf('Performing %i iterations of random cell addition',n_searches));
    for i_search = 1:n_searches 
        h1 = waitbar(0,sprintf('Performing random cell additions with depth %i',search_depth));
        ...
        cells_chosen{i_search} = cell(length(timecourse_save_steps),1);
        ...
        for added_cell = 1:search_depth % :numel(population)  
            % Add random cell
            cell_new_ix = randi(length(cells_left)); 
            ...
            cells_chosen{i_search}{added_cell} = cells_picked;
