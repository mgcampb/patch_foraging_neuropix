...
figure()
binscatter(rew_coeff(abs(rew_coeff) > 0.2),time_coeff(abs(rew_coeff) > 0.2),30,'xlimits',[-2,2],'ylimits',[-2,2])
ylim([-2,2])
xlim([-2,2])
...
X_umap = run_umap(beta_all_norm','n_components',n_components,'n_neighbors',n_neighbors,'verbose','none');

%% 
gscatter(X_umap(:,1),X_umap(:,2),region,[],'o',10)
