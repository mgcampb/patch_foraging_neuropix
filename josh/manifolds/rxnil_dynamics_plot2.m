function rxnil_dynamics_plot2(X,tts,colors,pre_cue_ix,median_cue_len,post_leave_ix)
% Plot RXNil dynamics in 2d
    hold on
    for tt = tts
        % pre-cue
        plot(X{tt}(1,1:pre_cue_ix), ...
              X{tt}(2,1:pre_cue_ix),'linewidth',2,'color',[.5 .5 .5]) 
        % cue
        plot(X{tt}(1,pre_cue_ix:pre_cue_ix + median_cue_len), ...
              X{tt}(2,pre_cue_ix:pre_cue_ix + median_cue_len),'linewidth',2,'color',[.2 .7 .2]) 
        % trial 
        plot(X{tt}(1,pre_cue_ix + median_cue_len:end-post_leave_ix), ...
              X{tt}(2,pre_cue_ix + median_cue_len:end-post_leave_ix),'linewidth',2,'color',colors{tt}) 
        ...
        plot(X{tt}(1,time_ticks),X{tt}(2,time_ticks), ... 
        ...
        % add mark for reward
        if mod(tt,2) == 0
            plot(X{tt}(1,(pre_cue_ix + median_cue_len)+50),X{tt}(2,(pre_cue_ix + median_cue_len)+50), ... 
                  'kd', 'markerSize', 15, 'markerFaceColor',colors{tt}); 
        end 

            % add some marks to make the trajectories more interpretable 
        plot(X{tt}(1,1),X{tt}(2,1), ...
              'ko', 'markerSize', 15, 'markerFaceColor',[.5 .5 .5]);
        plot(X{tt}(1,pre_cue_ix),X{tt}(2,pre_cue_ix), ...
              'ko', 'markerSize', 15, 'markerFaceColor',[.2 .7 .2]);  

        % add O at start of trial
        plot(X{tt}(1,pre_cue_ix + median_cue_len),X{tt}(2,pre_cue_ix + median_cue_len), ... 
              'ko', 'markerSize', 15, 'markerFaceColor',colors{tt});
        % add X at end of trial
        plot(X{tt}(1,end-post_leave_ix),X{tt}(2,end-post_leave_ix), ... 
              'ko', 'markerSize', 15, 'markerFaceColor',colors{tt});
        % add X at end of trial
        plot(X{tt}(1,end-post_leave_ix),X{tt}(2,end-post_leave_ix), ... 
