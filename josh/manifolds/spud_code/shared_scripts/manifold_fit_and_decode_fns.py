...
class PiecewiseLinearFit:
    ...
    def __init__(self, data_to_fit, params):
        ...
        self.make_loop = params['make_loop'] # call fhf.loop_knots to make ring?
        self.saved_knots = []
    ...
    def fit_data(self, fit_params):
        ...
        def cost_fn(flat_knots, penalty_params = fit_params):
            ...
            if self.make_loop == True:
                ...
                loop_knots = np.reshape(flat_knots.copy(), (self.nKnots, self.nDims))
...
def decode_from_passed_fit(data_to_decode, fit_coords, fit_curve, ref_angles):
    ...
    # JS: get rid of mult by 2pi .. maybe ?
    # unshft_coords = fhf.get_closest_manifold_coords(fit_curve,fit_coords, data_to_decode)
    unshft_coords = fhf.get_closest_manifold_coords(fit_curve,
        2*np.pi*fit_coords, data_to_decode)
    dec_angle, mse, shift, flip = af.shift_to_match_given_trace(unshft_coords,ref_angles)
