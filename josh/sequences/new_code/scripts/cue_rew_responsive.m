...
col_names = ["Mouse","Session","CellID","Region","GLM_Cluster","pValue_cuePatch","pValue_cue","pValue_patch", ... 
             "Cue_peak_pos","Cue_peak_neg","Rew0_peak_pos","Rew0_peak_neg","Rew1plus_peak_pos","Rew1plus_peak_neg","Rew1plus_peak_pos_sigOrNot","Rew1plus_peak_neg_sigOrNot"]; 
% col_types = arrayfun(@(x) 'double',1:14,'UniformOutput',false); 
col_types = ["string","string","double","string","double","double","double","double","double","double","double","double","double","double","double","double"];
transients_table = table('Size',[sum(s_nNeurons),16],'VariableTypes',col_types,'VariableNames',col_names);
...
colors = lines(2);
colors = [colors(1,:) ; max(0,colors(1,:) - .25) ; min(1,colors(1,:) + .15) ; colors(2,:); max(0,colors(2,:) - .25) ; min(1,colors(2,:) + .15)];
...
% title("Cortex")
% xlabel("Mean FR Off Patch (Hz)");ylabel("Mean FR During Proximity Cue (Hz)");
% legend("Non-Significant","p < 0.01 Lower FR","p < 0.01 Higher FR")

subplot(2,1,1) ; hold on
gscatter(mean_fr_mat_offPatch_pooled(cortex_binary),mean_fr_mat_patch_pooled(cortex_binary),sig_patch(cortex_binary),colors(1:3,:),'.oo',[5 5 5],'HandleVisibility','off');
h = refline(1,0); xlim([0 60]);ylim([0 60]);
h.LineStyle = '--';h.Color = 'k'; h.LineWidth = 3;
title("PFC")
xlabel("Mean FR Off Patch (Hz)");ylabel("Mean FR On Patch (Hz)");
legend("Non-Significant","p < 0.01 Lower FR","p < 0.01 Higher FR")
set(gca,'fontsize',16)
...
gscatter(mean_fr_mat_offPatch_pooled(~cortex_binary),mean_fr_mat_patch_pooled(~cortex_binary),sig_patch(~cortex_binary),colors(4:6,:),'.oo',[5 5 5],'HandleVisibility','off');
h = refline(1,0); xlim([0 60]);ylim([0 60]);
h.LineStyle = '--';h.Color = 'k'; h.LineWidth = 3;
legend("Non-Significant","p < 0.01 Lower FR","p < 0.01 Higher FR")
title("Sub-PFC")
...
alpha = .01;  
pValues_full = [pValue_cue pValue_patch]; % [pValue_cuePatch 
prop_sig = nan(4,2); 
for iVar = 1:2
    prop_sig(iVar,1) = mean(pValues_full(region == "PFC",iVar) < alpha); 
    prop_sig(iVar,2) = mean(pValues_full(region == "Sub-PFC",iVar) < alpha); 
    prop_sig(iVar+2,1) = mean(pValues_full(region == "PFC",iVar) > 1 - alpha);  
    prop_sig(iVar+2,2) = mean(pValues_full(region == "Sub-PFC",iVar) > 1- alpha); 
end
...
%% visualize shuffle results
labels = ["Sig Lower FR Cue","Sig Lower FR Patch" ...
          "Sig Higher FR Cue","Sig Higher FR Patch"];
        
figure();hold on
b = barh(prop_sig,'FaceColor','flat','linewidth',2); 
b(1).CData = colors(1,:); 
b(2).CData = colors(4,:);
yticks(1:4);yticklabels(labels) 
legend("PFC","Sub-PFC") 
...
% end
ytickangle(45);
xlabel("Proportion of Significant Persistent Activity Cells") 
...
pvalue_peth = cell(3,2);  
pos_peak_ix = cell(3,1); 
neg_peak_ix = cell(3,1); 
pos_peak_ix_sigOrNot = cell(3,1); 
neg_peak_ix_sigOrNot = cell(3,1); 
for iVar = 1:3 
    taskvar_peth{iVar} = nan(sum(s_nNeurons),length(var_bins{iVar})-1);  
    pvalue_peth{iVar,1} = nan(sum(s_nNeurons),length(var_bins{iVar})-1); 
    pvalue_peth{iVar,2} = nan(sum(s_nNeurons),length(var_bins{iVar})-1); 
    pos_peak_ix{iVar} = nan(sum(s_nNeurons),2); % [train test] 
    neg_peak_ix{iVar} = nan(sum(s_nNeurons),2); % [train test] 
    pos_peak_ix_sigOrNot{iVar} = nan(sum(s_nNeurons),2); % [train test] 
    neg_peak_ix_sigOrNot{iVar} = nan(sum(s_nNeurons),2); % [train test] 
end

transient_opt.nShuffles = 1; 
disp("changed this to one shuffle for peth making")
glm
counter = 0;  
...
for mIdx = 1:numel(mouse_grps)
    for i = 1:numel(mouse_grps{mIdx})   
        ...
        pos_peak_ix_sigOrNot{1}(i_start:i_end,1) = transient_struct_tmp.peak_ix_pos_sigOrNot;
        neg_peak_ix_sigOrNot{1}(i_start:i_end,1) = transient_struct_tmp.peak_ix_neg_sigOrNot;
        taskvar_peth{1}(i_start:i_end,:) = taskvar_peth_cell{1}; 
        pvalue_peth{1,1}(i_start:i_end,:) = pvalue_peth_cell{1};  % pos
        pvalue_peth{1,2}(i_start:i_end,:) = pvalue_peth_cell{2};  % neg
        ...
        pos_peak_ix_sigOrNot{1}(i_start:i_end,2) = transient_struct_tmp.peak_ix_pos_sigOrNot;
        neg_peak_ix_sigOrNot{1}(i_start:i_end,2) = transient_struct_tmp.peak_ix_neg_sigOrNot;
        ...
        for iVar = 2:3
            ...
            pos_peak_ix_sigOrNot{iVar}(i_start:i_end,1) = transient_struct_tmp.peak_ix_pos_sigOrNot;
            neg_peak_ix_sigOrNot{iVar}(i_start:i_end,1) = transient_struct_tmp.peak_ix_neg_sigOrNot;
            taskvar_peth{iVar}(i_start:i_end,:) = taskvar_peth_cell{iVar};  
            disp([mIdx i length(find(isnan(taskvar_peth_cell{iVar})))])
            pvalue_peth{iVar,1}(i_start:i_end,:) = pvalue_peth_cell{iVar,1}; 
            pvalue_peth{iVar,2}(i_start:i_end,:) = pvalue_peth_cell{iVar,2}; 
            transient_struct_tmp = driscoll_transient_discovery2(fr_mat_trials{mIdx}{i},task_vars_trialed{mIdx}{i},sort_trials,tbin_ms,var_bins,transient_opt);
            ...
            pos_peak_ix_sigOrNot{iVar}(i_start:i_end,2) = transient_struct_tmp.peak_ix_pos_sigOrNot;
            neg_peak_ix_sigOrNot{iVar}(i_start:i_end,2) = transient_struct_tmp.peak_ix_neg_sigOrNot;
        end  
        counter = counter + 1; 
...
neg_peak_ix_roi = cell(3,1);  
taskvar_peth_roi = cell(3,1);  
pvalue_peth_roi = cell(3,1);  
pos_peak = true;
for iVar = 1:3 
    pos_peak_ix_roi{iVar} = pos_peak_ix{iVar}(region_bool,:); 
    neg_peak_ix_roi{iVar} = neg_peak_ix{iVar}(region_bool,:);
    taskvar_peth_roi{iVar} = taskvar_peth{iVar}(region_bool,:);
    if pos_peak == true
        pvalue_peth_roi{iVar} = pvalue_peth{iVar,1}(region_bool,:);
    else
        pvalue_peth_roi{iVar} = pvalue_peth{iVar,2}(region_bool,:);
    end
end
...
%     pvalue_peth_roi_nonNan = pvalue_peth_roi{iVar}(both_nonNan,:);
    if pos_peak == true
        ...
        [~,train_sort] = sort(neg_peak_ix_roi{iVar}(:,1));  
        train_sort = train_sort(ismember(train_sort,find(~isnan(neg_peak_ix_roi{iVar}(:,1))))); % get rid of non significant cells
        [~,test_sort] = sort(neg_peak_ix_roi{iVar}(:,2));  
        test_sort = test_sort(ismember(test_sort,find(~isnan(neg_peak_ix_roi{iVar}(:,2))))); % get rid of non significant cells
    end
    figure() 
%     ax(1) = subplot(2,2,1);
...
for iVar = 1:3
    figure()
    if pos_peak == true
        scatter(.05 * rand(length(pos_peak_ix_roi{iVar}),1) + pos_peak_ix_roi{iVar}(:,1),.05 * rand(length(pos_peak_ix_roi{iVar}),1) + pos_peak_ix_roi{iVar}(:,2),'.')   
        both_nonNan = all(~isnan(pos_peak_ix_roi{iVar}),2);
        [r,p] = corrcoef(pos_peak_ix_roi{iVar}(both_nonNan,1),pos_peak_ix_roi{iVar}(both_nonNan,2));
    else
        scatter(.05 * rand(length(neg_peak_ix_roi{iVar}),1) + neg_peak_ix_roi{iVar}(:,1),.05 * rand(length(neg_peak_ix_roi{iVar}),1) + neg_peak_ix_roi{iVar}(:,2),'.')   
        both_nonNan = all(~isnan(neg_peak_ix_roi{iVar}),2);
        [r,p] = corrcoef(neg_peak_ix_roi{iVar}(both_nonNan,1),neg_peak_ix_roi{iVar}(both_nonNan,2));
    end
...
transients_table_gmm_dat = load('/Users/joshstern/Documents/UchidaLab_NeuralData/patch_foraging_neuropix/josh/structs/transients_table_gmm.mat');
transients_table_gmm = transients_table_gmm_dat.transients_table;
glm_cluster = transients_table_gmm.gmm_cluster; 

peak_bool = [0 1 0 1];
% just to control fineness of how we are dividing up the subplot
n_cols = 4; 
n_rows = 8;
figure()
for i_cluster = 1:4  
    ...
    subplot(n_rows,n_cols,i_cluster + (1:n_cols:n_cols*n_rows) - 1)
...
%         else % split by waveform for cluster 2
%             keep_pos_ns = find(glm_cluster == i_cluster & ~isnan(pos_peak_ix{3}(:,2)) & numeric_waveform_types == 1);
%             cluster_peaks_ns = pos_peak_ix{3}(keep_pos_ns,2); 
%             [~,cluster_peaksort_ns] = sort(cluster_peaks_ns);
%             cluster_peth_pos_ns = taskvar_peth{3}(keep_pos_ns,:);
%             keep_pos_rs = find(glm_cluster == i_cluster & ~isnan(pos_peak_ix{3}(:,2)) & numeric_waveform_types == 2); 
%             cluster_peaks_rs = pos_peak_ix{3}(keep_pos_rs,2); 
%             [~,cluster_peaksort_rs] = sort(cluster_peaks_rs);
%             cluster_peth_pos_rs = taskvar_peth{3}(keep_pos_rs,:);
%             prop_ns = length(cluster_peaks_ns)  / (length(cluster_peaks_ns) + length(cluster_peaks_rs));
%             prop_ns_round = round(prop_ns * n_rows) / n_rows; 
%             
%             max_row_ns = max(1,floor(prop_ns_round * n_cols * n_rows));
%             subplot(n_rows,n_cols,i_cluster + (1:n_cols:max_row_ns) - 1)
%             imagesc(flipud(zscore(cluster_peth_pos_ns(cluster_peaksort_ns,:),[],2)));
%             caxis([-3,3])
%             title(sprintf("Cluster %i",i_cluster))
%             set(gca,'fontsize',13)
%             xticks([])
%             ylabel(sprintf("Narrow-Spiking\n Peak Sort (X-Val)"))
% 
%             subplot(n_rows,n_cols,i_cluster + max_row_ns + (1:n_cols:floor((1-prop_ns_round) * n_cols*n_rows)) - 1)
%             imagesc(flipud(zscore(cluster_peth_pos_rs(cluster_peaksort_rs,:),[],2)));
%             ylabel(sprintf("Wide-Spiking\n Peak Sort (X-Val)"))
%             caxis([-3,3])
%         end
    else
...
%% Visualize cross validated peak sorts for GLM clusters, not caring whether or not the peak was significant
% this is a figure for the SfN
transients_table_gmm_dat = load('/Users/joshstern/Documents/UchidaLab_NeuralData/patch_foraging_neuropix/josh/structs/transients_table_gmm.mat');
transients_table_gmm = transients_table_gmm_dat.transients_table;
glm_cluster = transients_table_gmm.gmm_cluster; 
...
figure(); 
for i_cluster = 1:5
    keep_pos = glm_cluster == i_cluster; 
    keep_neg = glm_cluster == i_cluster; 
    cluster_peaks = pos_peak_ix_sigOrNot{3}(keep_pos,2); % transients_table.Rew1plus_peak_pos(keep_pos);
    cluster_troughs = neg_peak_ix_sigOrNot{3}(keep_neg,2);  % transients_table.Rew1plus_peak_neg(keep_neg);
    [~,cluster_peaksort] = sort(cluster_peaks); 
    [~,cluster_troughsort] = sort(cluster_troughs); 
...
%% Separate clusters by waveform 
glm_cluster = transients_table.gmm_cluster; 
peak_bool = [0 1 0 1]; 
figure()
for i_cluster = 1:4  
    keep_pos_ns = find(glm_cluster == i_cluster & ~isnan(pos_peak_ix{3}(:,2)) & numeric_waveform_types == 1); 
    keep_neg_ns = find(glm_cluster == i_cluster &  ~isnan(neg_peak_ix{3}(:,2)) & numeric_waveform_types == 1); 
    cluster_peaks_ns = pos_peak_ix{3}(keep_pos_ns,2); % transients_table.Rew1plus_peak_pos(keep_pos);
    cluster_troughs_ns = neg_peak_ix{3}(keep_neg_ns,2);  % transients_table.Rew1plus_peak_neg(keep_neg);
    [~,cluster_peaksort_ns] = sort(cluster_peaks_ns); 
    [~,cluster_troughsort_ns] = sort(cluster_troughs_ns); 
    cluster_peth_pos_ns = taskvar_peth{3}(keep_pos_ns,:);
    cluster_peth_neg_ns = taskvar_peth{3}(keep_neg_ns,:);
    
    keep_pos_rs = find(glm_cluster == i_cluster & ~isnan(pos_peak_ix{3}(:,2)) & numeric_waveform_types == 2); 
    keep_neg_rs = find(glm_cluster == i_cluster &  ~isnan(neg_peak_ix{3}(:,2)) & numeric_waveform_types == 2); 
    cluster_peaks_rs = pos_peak_ix{3}(keep_pos_rs,2); % transients_table.Rew1plus_peak_pos(keep_pos);
    cluster_troughs_rs = neg_peak_ix{3}(keep_neg_rs,2);  % transients_table.Rew1plus_peak_neg(keep_neg);
    [~,cluster_peaksort_rs] = sort(cluster_peaks_rs); 
    [~,cluster_troughsort_rs] = sort(cluster_troughs_rs); 
    cluster_peth_pos_rs = taskvar_peth{3}(keep_pos_rs,:);
    cluster_peth_neg_rs = taskvar_peth{3}(keep_neg_rs,:);
    
    subplot(2,4,i_cluster)
    if peak_bool(i_cluster) == true 
        imagesc(flipud(zscore(cluster_peth_pos_ns(cluster_peaksort_ns,:),[],2)));
%         imagesc(flipud(cluster_peth_pos_ns(cluster_peaksort_ns,:)./ max(cluster_peth_pos_ns(cluster_peaksort_ns,:),[],2)));
        caxis([-3,3])
        title(sprintf("Cluster %i",i_cluster))
        ylabel("Peak Sort (X-Val)")
    else
        imagesc(flipud(zscore(cluster_peth_neg_ns(cluster_troughsort_ns,:),[],2)));
%         imagesc(flipud(cluster_peth_neg_ns(cluster_troughsort_ns,:) ./ max(cluster_peth_neg_ns(cluster_troughsort_ns,:),[],2)));
        caxis([-3,3])
        title(sprintf("Cluster %i",i_cluster))
        ylabel("Trough Sort (X-Val)")
    end 
    ...
    if peak_bool(i_cluster) == true 
        imagesc(flipud(zscore(cluster_peth_pos_rs(cluster_peaksort_rs,:),[],2)));
%         imagesc(flipud(cluster_peth_pos_rs(cluster_peaksort_rs,:)./ max(cluster_peth_pos_rs(cluster_peaksort_rs,:),[],2)));
%         caxis([-3,3])
        title(sprintf("Cluster %i",i_cluster))
        ylabel("Peak Sort (X-Val)")
    else
        imagesc(flipud(zscore(cluster_peth_neg_rs(cluster_troughsort_rs,:),[],2)));
%         imagesc(flipud(cluster_peth_neg_rs(cluster_troughsort_rs,:) ./ max(cluster_peth_neg_rs(cluster_troughsort_rs,:),[],2)));
%         caxis([-3,3])
        title(sprintf("Cluster %i",i_cluster))
        ylabel("Trough Sort (X-Val)")
    end 
...
keep_pos_ns = find(glm_cluster == i_cluster & ~isnan(pos_peak_ix{2}(:,2)) & numeric_waveform_types == 1);
cluster_peaks_ns = pos_peak_ix{2}(keep_pos_ns,2); % transients_table.Rew1plus_peak_pos(keep_pos);
[~,cluster_peaksort_ns_rew0] = sort(cluster_peaks_ns);
cluster_peth_pos_ns_rew0 = taskvar_peth{2}(keep_pos_ns,:);
keep_pos_rs = find(glm_cluster == i_cluster & ~isnan(pos_peak_ix{2}(:,2)) & numeric_waveform_types == 2); 
cluster_peaks_rs = pos_peak_ix{2}(keep_pos_rs,2); % transients_table.Rew1plus_peak_pos(keep_pos);
[~,cluster_peaksort_rs_rew0] = sort(cluster_peaks_rs);
cluster_peth_pos_rs_rew0 = taskvar_peth{2}(keep_pos_rs,:);
...
figure() 
subplot(2,2,1)
imagesc(flipud(zscore(cluster_peth_pos_ns_rew0(cluster_peaksort_ns_rew0,:),[],2)));
caxis([-3,3])
title(sprintf("Cluster %i NS \n Reward t = 0",i_cluster))
ylabel("Peak Sort (X-Val)")
xticks([10 20 30 40])
xticklabels(["0.5" "1.0" "1.5" "2.0"])
xlabel("Time since reward (sec)")
set(gca,'fontsize',13)
subplot(2,2,2)
imagesc(flipud(zscore(cluster_peth_pos_rs_rew0(cluster_peaksort_rs_rew0,:),[],2)));
caxis([-3,3])
title(sprintf("Cluster %i RS \n Reward t = 0",i_cluster))
ylabel("Peak Sort (X-Val)")
xticks([10 20 30 40])
xticklabels(["0.5" "1.0" "1.5" "2.0"])
xlabel("Time since reward (sec)")
...
%         imagesc(log(pvalue_peth{iVar}(glm_bool,:)))  ; colormap('bone') 
        imagesc(zscore(taskvar_peth{iVar}(glm_bool,:),[],2)); 
        if iVar > 1
...
%% plot mean cue responsivity per cluster

figure() ;hold on
colors = lines(4);
for glm_cluster = 1:4 
    glm_bool = transients_table.GLM_Cluster == glm_cluster; 
    shadedErrorBar(1:size(taskvar_peth{1},2),mean(zscore(taskvar_peth{1}(glm_bool,:),[],2)),1.96 * std(zscore(taskvar_peth{1}(glm_bool,:),[],2)) / sqrt(length(find(glm_bool))),... 
                    'lineProps',{'color',colors(glm_cluster,:),'linewidth',2})
%     shadedErrorBar(1:size(taskvar_peth{1},2),mean(taskvar_peth{1}(glm_bool,:)),1.96 * std(taskvar_peth{1}(glm_bool,:)) / sqrt(length(find(glm_bool))),... 
%                     'lineProps',{'color',colors(glm_cluster,:),'linewidth',2})            
end 
legend("GLM Cluster 1","GLM Cluster 2","GLM Cluster 3","GLM Cluster 4")
xticks(1:10:numel(var_bins{iVar}))
xticklabels(var_bins{iVar}(1:10:end)) 
ylabel("Mean Z-Scored Response")  
% ylabel("Mean FR")
xlabel("Time Since Proximity Cue")
set(gca,'fontsize',14)
...
%% Proportion positive / negative peaks per cluster 
proportion_sig_peak_trough = nan(4,2) ;
for i_gmm_cluster = 1:4
    proportion_sig_peak_trough(i_gmm_cluster,1) = mean(~isnan(transients_table.Rew1plus_peak_pos(transients_table.gmm_cluster == i_gmm_cluster)));
    proportion_sig_peak_trough(i_gmm_cluster,2) = mean(~isnan(transients_table.Rew1plus_peak_neg(transients_table.gmm_cluster == i_gmm_cluster)));
end
rdBu = cbrewer('div','RdBu',3); 
figure()
set(gca,'colorOrder',rdBu([1 3],:))
b = bar(proportion_sig_peak_trough,'facecolor','flat') ;
b(1).CData = rdBu(1,:); 
b(2).CData = rdBu(3,:); 
legend(["Positive Transients","Negative Transients"])
ylabel("Proportion Significant Transient")
xticklabels(arrayfun(@(x) sprintf("Cluster %i",x),1:4))
...
glm_cluster = transients_table.gmm_cluster; 
glm_simesince_peth = taskvar_peth{3}(~isnan(transients_table.gmm_cluster),:); 
keep_pos = ~isnan(transients_table.Rew1plus_peak_pos) & ~isnan(transients_table.gmm_cluster);
glm_rew_peaks = transients_table.Rew1plus_peak_pos(keep_pos);
simesince_peth_pos = taskvar_peth{3}(keep_pos,:);
keep_neg = ~isnan(transients_table.Rew1plus_peak_neg) & ~isnan(transients_table.gmm_cluster);
...
for i_cluster = 1:4  
    ...
    if peak_bool(i_cluster) == true 
        ...
        title(sprintf("Cluster %i",i_cluster))
%         if i_cluster == 1
            ylabel("X-val Peak Sort")
%         end
    else
        imagesc(flipud(zscore(cluster_peth_neg(cluster_troughsort,:),[],2)));
        title(sprintf("Cluster %i",i_cluster))
%         if i_cluster == 1
            ylabel("X-val Trough Sort")
%         end
    end
    xticks([10 20 30 40])
    xticklabels(["0.5" "1.0" "1.5" "2.0"])
...
close all
ROI = "PFC";
glm_cluster = transients_table(transients_table.Region == ROI,:).gmm_cluster;
...
figure() 
subplot(1,2,1)
percentClu1 = mean(~isnan(rew1plus_peak_pos(glm_cluster == 1)));
percentClu2 = mean(~isnan(rew1plus_peak_pos(glm_cluster == 2)));
percentClu3 = mean(~isnan(rew1plus_peak_pos(glm_cluster == 3)));    
bar([percentClu1 percentClu2 percentClu3]) 
title(sprintf('Proportion of Significant Post-Rew0 Peaks Discovered \n Divided By GLM Cluster')) 
xticklabels(["Cluster 1","Cluster 2","Cluster 3"])
ylabel("Proportion of Cells With Significant Post-Rew0 Peak")
subplot(1,2,2) 
histogram(glm_cluster(~isnan(glm_cluster) & isnan(rew1plus_peak_pos)),'Normalization','Probability') 
...
%% peths for both signs of transient 
...
for i_cluster = 1:4  
    for pos_peak = 0:1
        ...
        subplot(2,4,4 * pos_peak + i_cluster)
        if pos_peak == true 
            imagesc(flipud(zscore(cluster_peth_pos(cluster_peaksort,:),[],2)));
            caxis([-3,3])
            title(sprintf("Cluster %i",i_cluster))
