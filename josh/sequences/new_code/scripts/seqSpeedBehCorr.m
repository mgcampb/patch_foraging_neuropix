...
calc_frOpt.preLeave_buffer = 500;  
calc_frOpt.cortex_only = true;
...
for sIdx = 1:numel(sessions)
    ...
    if isfield(dat,'anatomy') && exist(ramp_fname,'file')
        ...
        if exist('structs/driscoll_midresp.mat','file')
            load('structs/driscoll_midresp.mat')
        else
...
r_rampSlope = nan(numel(sessions),1); 
p_rampSlope = nan(numel(sessions),1);
...
r1uL_rampSlope_pooled = nan(numel(mouse_groups,1));
p1uL_rampSlope_pooled = nan(numel(mouse_groups,1));
...
r2uL_rampSlope_pooled = nan(numel(mouse_groups,1));
p2uL_rampSlope_pooled = nan(numel(mouse_groups,1));
...
r4uL_rampSlope_pooled = nan(numel(mouse_groups,1));
p4uL_rampSlope_pooled = nan(numel(mouse_groups,1));
...
%         legend("1 uL","2 uL","4 uL")
        ...
        % Ramp slope - PRT
        [r0,p0] = corrcoef(ramp_slope(one_rew_trials),prts(one_rew_trials));
        [~,p1] = corrcoef(ramp_slope(sm1rew),prts(sm1rew));
        [r2,p2] = corrcoef(ramp_slope(md1rew),prts(md1rew));
        [r3,p3] = corrcoef(ramp_slope(lg1rew),prts(lg1rew));
%         colors = cool(3);
%         figure();hold on
%         gscatter(ramp_slope(one_rew_trials),prts(one_rew_trials),rewsize(one_rew_trials),colors,'.')
%         xlabel("Fitted slope of mean ramp")
%         ylabel("PRT")
%         title(sprintf("%s Ramp Slope vs PRT (overall p = %f, 1uL p = %f, 2 uL p = %f, 4 uL p = %f)",session_title,p0(2),p1(2),p2(2),p3(2)))
%         legend("1 uL","2 uL","4 uL")
        r_rampSlope(sIdx) = r0(2);
        p_rampSlope(sIdx) = p0(2);
...
%         xlabel("Mean Mid-Responsive Neuron Activity")
    ...
    [r1_rampSlope,p1_rampSlope] = corrcoef(rampSlope1uL_pooled{m},prt1uL_pooled{m});
...
%     legend(session_titles_grp{m}) % days from this mice 
    ...
    r1uL_rampSlope_pooled(m) = r1_rampSlope(2);
    p1uL_rampSlope_pooled(m) = p1_rampSlope(2); 
    ...
    [r2_rampSlope,p2_rampSlope] = corrcoef(rampSlope2uL_pooled{m},prt2uL_pooled{m});
...
%     legend(session_titles_grp{m}) % days from this mice 
    ...
    r2uL_rampSlope_pooled(m) = r2_rampSlope(2);
    p2uL_rampSlope_pooled(m) = p2_rampSlope(2); 
    ...
    [r3_rampSlope,p3_rampSlope] = corrcoef(rampSlope4uL_pooled{m},prt4uL_pooled{m});
...
%     ylabel("PRT") 
%     mouseName = session_titles_grp{m}{1}(1:2);
%     title(sprintf("m%s 4 uL Slope of sequence progression vs PRT (p = %f,r = %f)",mouseName,p3_progSlope(2),r3_progSlope(2)))
%     legend(session_titles_grp{m}) % days from this mice  
%     % pooled seq-ramp
%     figure();hold on
...
%     legend(session_titles_grp{m}) % days from this mice 
    ...
    r4uL_rampSlope_pooled(m) = r3_rampSlope(2);
    p4uL_rampSlope_pooled(m) = p3_rampSlope(2); 
...
p_rampSlope = p_rampSlope(~isnan(p_rampSlope));
r_rampSlope = r_rampSlope(~isnan(r_rampSlope));
p_seqRamp = p_seqRamp(~isnan(p_seqRamp));
r_seqRamp = r_seqRamp(~isnan(r_seqRamp)); 

% how are these things related to the number of neurons per animal
n_seqNeurons = cellfun(@length,midresp); 
n_seqNeurons = n_seqNeurons(mPFC_sessions); % only mPFC sessions 
FR_decVar_cell = squeeze(struct2cell(FR_decVar))';
n_upAllNeurons = cellfun(@length,FR_decVar_cell(:,4)); 
n_upAllNeurons = n_upAllNeurons(mPFC_sessions); % only mPFC sessions

% how do correlation coefficients relate to number of neurons?
significance_labels = zeros(numel(r_seqProg),1);
significance_labels(p_seqProg < .05) = 1;
colors = [0 0 0;1 0 0];
figure()
yline(0,'linewidth',1.5)
xline(0,'linewidth',1.5)
hold on
p = gscatter(n_seqNeurons,r_seqProg,significance_labels,colors);
legend(p(1:2),"No significance", sprintf("Sequence Progression vs PRT p < .05"))
grid()
ylim([-1,1])
xlabel("Number of Sequence Neurons")
ylabel("Sequence Progression vs PRT corr")
title("Effect of Number of Sequence Neurons on Sequence Progression PRT corr") 

% how do correlation coefficients relate to number of neurons?
significance_labels = zeros(numel(r_seqProg),1);
significance_labels(p_rampSlope < .05) = 1;
colors = [0 0 0;1 0 0];
figure()
yline(0,'linewidth',1.5)
xline(0,'linewidth',1.5)
hold on
p = gscatter(n_upAllNeurons,r_rampSlope,significance_labels,colors);
legend(p(1:2),"No significance", sprintf("Ramp Slope vs PRT p < .05"))
grid()
ylim([-1,1])
xlabel("Number of Ramping Neurons")
ylabel("Ramp Slope vs PRT corr")
title("Effect of Number of Ramping Neurons on Ramp Slope PRT corr")

% results for progression speed vs rampSlope across all rewsizes  
scatterCorr(r_seqProg,p_seqProg,r_rampSlope,p_rampSlope,"Seq Progression Slope vs PRT","Ramp Slope vs PRT");
scatterCorr(r_rampSlope,p_rampSlope,r_seqRamp,p_seqRamp,"Ramp Slope vs PRT","Sequence Prog Slope vs Ramp Slope"); 
...
barWithinRewCorr(corrSeqProg,pSeqProg,"Slope of Sequence Progression","PRT",mouse_names); 
...
barWithinRewCorr(corrMeanMid,pMeanMid,"Z-scored Sequence Neuron Activity","PRT",mouse_names);  

% Ramp-PRT correlation
corrRampSlope = [r1uL_rampSlope_pooled' r2uL_rampSlope_pooled' r4uL_rampSlope_pooled']; 
pRampSlope = [p1uL_rampSlope_pooled' p2uL_rampSlope_pooled' p4uL_rampSlope_pooled']; 
barWithinRewCorr(corrRampSlope,pRampSlope,"Slope of Ramping Activity","PRT",mouse_names); 

% Last, pooled sequence-ramp correlations 
...
barWithinRewCorr(corrSeqRamp,pSeqRamp,"Slope of Sequence Progression","Slope of Mean Ramping Activity",mouse_names);
