...
if ismember(1,variables)
    ...
    [speed_y,speed_x] = spline_1d_plot(param1,ctl_pts_all{1},s);
end
...
if plotfig
    ...
    if ismember(var_k,variables)
        ...
        plot(speed_x,exp(speed_y)*prod(scale_factor)*exp(b0)/dt,'r','linewidth',2);
        box off
        xlabel('speed')
        ylabel('spikes/s')
        axis tight
        tuning_curves{var_k} = exp(speed_y')*exp(b0)*prod(scale_factor)/dt;
        vector_x = speed_x; vector_y = tuning_curves{var_k};
        %plot_special_points
        title('LNP speed tuning')
        legend({'tc','lnp'});
    end
    
    set(gcf, 'Position', [12.2000 12.200 500 800]);
end
return