...
% what session and reward size to analyze:
...
var_name = [var_name,'TimeOnPatch','TotalRew','TimeSinceRew'];
...
good_cells_josh_sort = good_cells(neuron_order);
good_cells_josh_sort_filt = good_cells_josh_sort(ismember(good_cells_josh_sort,good_cells_sig));
josh_order = nan(size(good_cells_josh_sort_filt));
for i = 1:numel(josh_order)
    josh_order(i) = find(good_cells_sig==good_cells_josh_sort_filt(i));
end
...
colormap(jet);
subplot(1,3,1);
imagesc(tkern,1:size(ypred,2),ypred(:,josh_order)');
cb = colorbar;
xlabel('Time since reward');
ylabel('Neuron, Josh sort');
title('Model fit (kernels only)')
ylabel(cb,'Multiplicative firing rate change');

% create peth
fr_mat = spikecounts_filt(:,sig_cells)/opt.tbin;
for i = 1:size(fr_mat,2)
    fr_mat(:,i) = gauss_smoothing(fr_mat(:,i),opt.smooth_sigma_fr/opt.tbin);
end
keep = false(size(t));
...
x = t_since_rew(keep);
tkernedge = linspace(0,2,41); % -opt.tbin/2:opt.tbin:opt.basis_length+opt.tbin/2;
[~,~,bin]=histcounts(x,tkernedge);
psth = nan(max(bin),size(fr_mat,2));
for i = 1:max(bin)
    psth(i,:) = mean(fr_mat(bin==i,:));
end
psth = zscore(psth); % z-score each row (like josh)

subplot(1,3,2);
imagesc(tkern,1:size(psth,2),psth(:,josh_order)');
cb = colorbar;
xlabel('Time since reward');
ylabel('Neuron, Josh sort');
ylabel(cb,'z-scored firing rate');
title(sprintf('%duL trials, z-score within row',opt.rew_size));

% Full model
subplot(1,3,3);
ypred = exp([ones(size(X_full,1),1) X_full] * beta_sig);
psth = nan(max(bin),size(ypred,2));
for i = 1:max(bin)
    psth(i,:) = mean(ypred(bin==i,:));
end
keep = std(psth)>1e-10;
psth(:,keep)=zscore(psth(:,keep)); % z-score each row (like josh)
psth(:,~keep) = psth(:,~keep)-mean(psth(:,~keep));

imagesc(tkern,1:sum(sig_cells),psth(:,josh_order)');
cb = colorbar;
xlabel('Time since reward');
ylabel('Neuron, Josh sort');
title('C. Full model')
