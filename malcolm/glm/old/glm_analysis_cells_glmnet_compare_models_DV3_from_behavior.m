...
X_this = [t_on_patch' t_since_last_rew' DV3'];
A = [A, {X_this}];
grp_name = [grp_name,'DVs'];
var_name = [var_name,'TimeOnPatch','TimeSinceLastRew','DV3'];
...
% base model
X_mdl{1} = X_all(:,~strcmp(var_name,'TimeOnPatch') & ~strcmp(var_name,'DV3') & ...
    ~strcmp(var_name,'TimeSinceLastRew'));

% model 1
X_mdl{2} = X_all(:,~strcmp(var_name,'DV3') & ~strcmp(var_name,'TimeSinceLastRew'));

% model 2
X_mdl{3} = X_all(:,~strcmp(var_name,'TimeOnPatch') & ~strcmp(var_name,'DV3'));

% model 3
X_mdl{4} = X_all(:,~strcmp(var_name,'TimeOnPatch') & ~strcmp(var_name,'TimeSinceLastRew'));
...
hfig.Name = sprintf('%s single cell model comparison (LLH_model - LLH_mean)_DV3 from behavioral fit',opt.session);
counter = 1;
