...
opt.rew_size = [1 2 4];
opt.data_set = 'mc';
opt.vel_thresh = 2.5; % velocity threshold for computing PRTs
...
hfig.Name = sprintf('PRTs all recording sessions VELOCITY THRESHOLD=%0.1f %s cohort.png',opt.vel_thresh,opt.data_set);
...
for session_idx = 1:numel(session_all)
    ...
    %% re-compute PRTs based on velocity threshold
    patchLeave_vel = nan(size(dat.patchCSL,1),1);
    t_run = dat.velt(dat.vel>opt.vel_thresh);
    for pIdx = 1:numel(patchLeave_vel)
        patchLeave_vel(pIdx) = min(t_run(t_run>dat.patchCSL(pIdx,2)));
    end
    prt = patchLeave_vel-dat.patchCSL(:,2);
    ...
    for i = 1:3
        counter = -1;
        for j = 1:3
            y_this = prt(rew_size_all==opt.rew_size(i) & N0_all==4-j);
